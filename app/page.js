...
export default function Home(){
 const [screen,setScreen]=useState('home'),[cards,setCards]=useState([]),[concept,setConcept]=useState(null),[picked,setPicked]=useState(null),[seen,setSeen]=useState([]),[known,setKnown]=useState([]),[world,setWorld]=useState('psychology');
 ...
 const worldConcepts=concepts.filter(c=>(c.world||'psychology')===world);\n const discover=(nextWorld=world)=>{const pool=concepts.filter(c=>(c.world||'psychology')===nextWorld);setWorld(nextWorld);setCards(diverseSample(pool,5,nextWorld===world?cards.map(x=>x.id):[]));setScreen('browse');setConcept(null);setPicked(null)};
 ...
 {screen==='worlds'&&<section className="wrap"><div className="eyebrow">WORLDS</div><h2>What are you curious about?</h2><p className="muted">One engine, different ways of learning. Visuals appear when they actually help.</p><div className="worldgrid">{worlds.map((w,i)=><button key={w.id} className={'world '+(['psychology','neuroscience','body'].includes(w.id)?'':'locked')} onClick={()=>['psychology','neuroscience','body'].includes(w.id)&&discover(w.id)}><small>0{i+1} · {w.target} concepts</small><h3>{w.name}</h3><p>{w.desc}</p><b>{['psychology','neuroscience','body'].includes(w.id)?'Explore →':'Planned'}</b></button>)}</div></section>}
 {screen==='browse'&&<section className="wrap browse"><div className="browsehead"><div><div className="eyebrow">{worlds.find(w=>w.id===world)?.name.toUpperCase()} · DISCOVERY</div><h2>Which one pulls you in?</h2></div><button className="ghost" onClick={()=>setCards(diverseSample(worldConcepts,5,cards.map(x=>x.id)))}>Shuffle ↻</button></div><div className="cardstack">{cards.map((c,i)=><button className="conceptcard" onClick={()=>open(c)} key={c.id}><span className="num">0{i+1}</span><div><small>{c.pool}</small><h3>{c.name}</h3><p>{c.hook}</p></div><b>→</b></button>)}</div><p className="hint">There is no best choice. Follow the question you want answered.</p></section>}
