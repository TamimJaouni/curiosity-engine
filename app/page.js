...
export default function Home(){
 const [screen,setScreen]=useState('home'),[cards,setCards]=useState([]),[concept,setConcept]=useState(null),[picked,setPicked]=useState(null),[seen,setSeen]=useState([]),[known,setKnown]=useState([]),[world,setWorld]=useState('psychology'),[category,setCategory]=useState('');
 ...
 const chooseWorld=nextWorld=>{setWorld(nextWorld);setCategory('');setConcept(null);setPicked(null);setScreen('categories')};
 const chooseCategory=label=>{const pool=concepts.filter(c=>(c.world||'psychology')===world&&c.pool===label);setCategory(label);setCards(diverseSample(pool,5));setScreen('browse');setConcept(null);setPicked(null)};
 const discover=()=>{const pool=category?worldConcepts.filter(c=>c.pool===category):worldConcepts;setCards(diverseSample(pool,5,cards.map(x=>x.id)));setScreen('browse');setConcept(null);setPicked(null)};
 ...
 {screen==='browse'&&<section className="wrap browse"><div className="browsehead"><div><div className="eyebrow">{category?worlds.find(w=>w.id===world)?.name.toUpperCase()+' · '+category.toUpperCase():worlds.find(w=>w.id===world)?.name.toUpperCase()+' · DISCOVERY'}</div><h2>Which one pulls you in?</h2></div><button className="ghost" onClick={()=>{const pool=category?worldConcepts.filter(x=>x.pool===category):worldConcepts;setCards(diverseSample(pool,5,cards.map(x=>x.id)))}}>Shuffle ↻</button></div><div className="cardstack">{cards.map((c,i)=><button className="conceptcard" onClick={()=>open(c)} key={c.id}><span className="num">0{i+1}</span><div><small>{c.pool}</small><h3>{c.name}</h3><p>{c.hook}</p></div><b>→</b></button>)}</div><p className="hint">There is no best choice. Follow the question you want answered.</p></section>}
